...
class EnquiryViewTestCase(test_utils.BaseEnquiryTestCase):
    ...
    def assert_enquiry_equals_csv_row(self, enquiry, csv_row):
        ...
        if enquiry.enquirer:
            for enquirer_key in utils.ENQUIRER_FIELD_NAMES:
                ...
                self.assertEqual(
                    csv_row[csv_row_key],
                    str(enquirer_val) if enquirer_val is not None else "",
                )
    ...
    def test_enquiry_import_post_blank(self):
        ...
        self.assertEqual(
            error_message, "No file was selected. Choose a file to upload."
        )
    ...
    def test_enquiry_csv_format_content_disposition(self):
        ...
        assert (
            ...
            == "attachment; filename=rtt_enquiries_export.csv"
        )
    ...
    def test_enquiry_list_filtered_unassigned_html(self):
        ...
        # owner unassigned
        response = self.client.get(
            reverse("index"), {"owner__id": "UNASSIGNED"}, **headers
        )
        soup = BeautifulSoup(response.content, "html.parser")
        enquiry_els = soup.select(".entity__list-item")
...
class TestAPIEnquiries:
    ...
    def test_enquiries_pagination(self, api_client, settings):
        ...
        sender = _auth_sender("test", "test-key", url, "GET")
        response = api_client.get(
            url, content_type="", HTTP_AUTHORIZATION=sender.request_header
        )
        data = response.json()
