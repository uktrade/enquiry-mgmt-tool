...
class EnquiryViewTestCase(TestCase):
    ...
    def test_enquiry_detail_template(self):
        """Test the template 
        is using the right variables
        to show enquiry data"""
        enquiry = EnquiryFactory()
        response = self.client.get(reverse("enquiry-detail", kwargs={"pk": enquiry.id}))
        self.assertContains(response, enquiry.company_name)
        self.assertContains(response, enquiry.enquiry_stage)
        self.assertContains(response, enquiry.country)
