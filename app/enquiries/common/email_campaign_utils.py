...
3. Collect a log of each such subscription to ensure the next run picks up from the last subscription
...
def process_latest_enquiries():
    ...
    """
    last_action_date = EnquiryActionLog.action_date_boundary(ref_data.EnquiryAction.EMAIL_CAMPAIGN_SUBSCRIBE)
...
def process_engaged_enquiries():
    """
    Collect all enquiries marked with the `EXIT_STAGE` which will cause them to be unsubscribed
    from the campaign.
    """
    last_action_date = EnquiryActionLog.action_date_boundary(ref_data.EnquiryAction.MARKED_RESPONSIVE)
